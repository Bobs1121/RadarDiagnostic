...
| D-PI-15 | **Source-only Pi 自动绑定当前 variant 索引** | 无录制数据的源码查询按有效配置 variant 调用 `CodeContextRefreshModule`，在该 variant 的 snapshot workspace 生成/复用当前 `code-context.v1` 和 `code-index.v1`；PiRunContext 校验两者 schema/path/hash、source root、snapshot 与 context→index 引用，Pi child bridge 将其限定传入 `code-analyze` / `code-context-read` / `event-code-path` / `code-gdb-plan` 并拒绝覆盖，也禁止该 scope 通过 tool `output` 写文件；禁止回落到共享或过期 CodeGraph。 |
| D-PI-16 | **Default variant 决定所有兼容路径与源码域** | `config.load_config()` 存在有效 `default_variant` 时，legacy `paths.*` 与 active `source_domains` 必须从该 variant 的 codebase/key files/DBC/source_context 派生；`default_project` 和 top-level legacy domains 只在没有可解析 variant 时作 fallback。信号链无显式 RTE 源时按当前 variant 唯一选择；缺失/歧义时返回 unavailable，不默认套用 GWM。 |
| D-PI-17 | **Source-only Pi 调查可恢复并与模型会话分离** | 无 case 的 source-only 一次性查询自动创建本地 AnalysisRun，并在当前 code context/index 校验后绑定 project/variant/source snapshot/index hash；默认 Pi session ID 稳定派生为 `pi-<AnalysisRun ID>`，与 ledger run ID 不同。只读源码工具的 Pi schema 隐藏文件型 output 参数，结果 inline 返回；工具状态和 ModuleResult 的 run 状态必须与 event result 与 ledger finalize 一致。 |
| D-PI-18 | **Pi ledger leaves 自动绑定当前调查 run** | PiBridge 向本次 child process 绑定 active AnalysisRun ID 和 ledger root；read/update/step/claim/hypothesis/experiment/user-observation leaves 自动注入这两个字段，模型不得指定不同 run 或 root。无 active run 时 fail-closed，已有 run 中禁止重复创建。Pi schema 将内部 binding 字段设为可省略，CLI 仍要求显式 run_id。 |
...
- `docs/archive/2026-09-17/technical/V4_PI_DRIVEN_ARCHITECTURE.md` — 顶层架构设计（已创建）
- `docs/archive/2026-09-17/technical/V4_DEVELOPMENT_PLAN.md` — 分片开发计划 S0-S7（已创建，作为拆分参考）
- `docs/archive/2026-09-17/technical/V4_PI_BASED_PLAN.md` — **基于 pi 的详细方案设计与实施计划（P0-P7，实施主线）**
- `docs/archive/2026-09-17/technical/V4_DESIGN_CONTEXT_AND_DECISIONS.md` — 本文件（需求与决策上下文）
...
- 2026-09-16 [D/R5] 用户强调低心智负担须贯穿整个产品，代码变量只是示例；要求结合旧文档现状/环境统一 Document-driven Development 套件。即时确认默认独立分析会话，以及 Pi 对话＋实时浏览器工作页作为首期目标。后续唯一导航为 [Gen6 DDD 入口](../../../technical/GEN6_AI_DOCUMENT_INDEX.md)，旧正式 PID attach 优先等默认策略按新设计裁决。

- 2026-09-16 [D/R3] 三轮需求已收敛：数据/对应版本环境就绪后十分钟完成 L1/L2（含必要回放/GDB），准备编译与授权后 L3 单独计时并保留端到端耗时；单 bag 不超过用户所述 10 GB。全局盘点后按需求灵活调查代表/差异事件；Pi 按需调用预先注册的原子工具并自主选择/升级仿真模式。后续按 [产品与验收基线](../../../technical/GEN6_AI_PRODUCT_BASELINE_2026-09-16.md) 及 [开发计划](../../../technical/GEN6_AI_DEVELOPMENT_PLAN_2026-09-16.md) 开发；所有新验收 pending，不构成执行授权。

- 2026-09-16 [D/R2] 典型输入为单 bag+问题，录制软件版本对应算法 branch/commit，并绑定车型/COEM。四雷达、多报警事件需隔离；HILMODEL 的点云/目标级路径依当前源码确认，目标级不证明 perception 执行。报告几何属性必须来源真实代码/数据；首期以问题数据回归，其他数据集回归后续接入。单 bag 十分钟完成分析为用户目标，计时与完成范围待确认。详见 [访谈第二轮与第三轮待答项](../../../technical/GEN6_AI_PRODUCT_DISCOVERY_2026-09-16.md)。

- 2026-09-16 [D] 新一轮用户访谈确认：小范围独立对话工具；基于 CR60 Light arbe 和算法子仓的 branch/COEM 管理；点云之后贯通 perception/situation/fct，不含 DSP。以“现象与场景还原→根因调查→授权修改验证”三层能力分别交付有效信息，不强制固定工具顺序。性能取舍或非问题判断须提供证据交由用户裁决；授权采用按任务范围、范围内自主推进方案。详见 [完整访谈、需求编号与待答问题](../../../technical/GEN6_AI_PRODUCT_DISCOVERY_2026-09-16.md)；此为产品需求，不是当前执行授权或实现完成声明。

- 2026-09-11 [D] 自主 rosbag 仿真、源码联合分析、报告、准确性与速度、原子工具和 Pi 编排保持为目标；按 [设计复核及 DDD 收口基线](SIMULATION_ANALYSIS_DDD_REVIEW_2026-09-11.md) 实施 P0～P4。确定性状态机只管理执行生命周期，AI 保留调查路径和根因推理职责。
- 2026-09-14 [D] Source-only Pi 自动按有效 variant 配置生成/复用当前 source context/index；source root、snapshot 和 code-index 文件 hash 必须绑定到 PiRunContext 与该次 child bridge，identity/schema/hash 不一致时停止，不能退回默认 CodeGraph。
- 2026-09-15 [D] 有有效 `default_variant` 时，兼容 `paths.*` 从该 variant 派生，旧 `default_project` 只作无 variant 的 fallback；variant 信号链 mapping 缺失或有歧义时标 unavailable，不套用 GWM 默认 RTE。
- 2026-09-15 [D/实施] `config.load_config()` 已按 D-PI-16 对齐 source/DBC/key/source_docs paths 与 variant `source_domains`；`trace_variable_chains` 绑定 selected COEM 并将 RTE+Tx companions 与 COEM/source hashes 纳入 cache signature，显式路径缺失/多 COEM 歧义都不再回退 GWM。
- 2026-09-15 [D/实施] Source-only Pi 现在自动持久化 AnalysisRun，以独立稳定 Pi session ID 恢复；生成的只读源码工具 schema 不暴露 output 文件路径，AnalysisRun tool step 和 finalize 状态与真实 Pi event 一致。
- 2026-09-15 [D/实施] Pi ledger leaves 现在绑定当前 AnalysisRun/root，Pi tool bridge 自动注入 ID/path 并拒绝跨 run/root 覆盖；Hypothesis/DebugExperiment tool call 可省略内部字段。
- 2026-09-15 [D/实施] Analysis ledger leaf tools 由 bridge 自动绑定当前 AnalysisRun/root 并拒绝跨 run/path 覆盖；Pi 不需要记住内部 run_id 或 ledger 路径，CLI 仍保留显式参数。
- 2026-09-11 [R] 当前工作树存在 Pi 感知能力选择、阶段 producer、完成确认、输入解码和身份复核缺口；本地契约 green 不代表真实感知闭环。此前 ready/verified 须按各自检查范围解释。

- 2026-09-10 [D] 用户要求保留 ADAS 分析，并加入点云驱动的前级感知流程、数据/代码联合分析和报告设计；DDD 仍指文档驱动开发。
- 2026-09-10 [R] 只读检查远端当前 symmetry `postProcess.c`，主要感知阶段位于 `#if 0 == HILMODEL`，当前头文件为 2；当前成功目标注入回放不足以证明感知重新执行。
- 2026-09-10 [TBD] 独立点云构建 profile、stage trace、lineage 和报告契约均为拟议设计，需按 [点云感知 DDD 方案](POINT_CLOUD_PERCEPTION_DDD_PROPOSAL_2026-09-10.md) 的 PC-AC 实施验收。
...
| 2026-08-24 | [R] 实际 ROS/rosbag 诊断工具框架探索：补充远程 session/replay/debug adapter、bag/msgdef/provenance、radar/target/frame 选择、100-200 帧连续预热契约、GDB target/PID 映射、条件证据五态、A2L/XCP 调参边界和失败降级规则；具体记录见 `docs/archive/2026-09-17/ros_debug/ROS_DEBUG_DIAGNOSTIC_TOOL_FRAMEWORK.md` 与样例探索附录。 |

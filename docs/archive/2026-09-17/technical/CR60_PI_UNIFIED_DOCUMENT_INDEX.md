...
新一轮产品基线（2026-09-16）：三轮核心需求已确认，后续开发先读下列文档。三层交付、首期范围、授权修改、模式选择和性能口径有冲突时以本轮基线为准；旧技术契约与历史验收证据按实际适用范围保留。

1. [Gen6 AI 产品与验收基线](../../../technical/GEN6_AI_PRODUCT_BASELINE_2026-09-16.md)：产品行为、Pi/工具/arbe 职责、报告、授权、十分钟目标与 G6-AC01～11。
2. [Gen6 AI 开发与验收计划](../../../technical/GEN6_AI_DEVELOPMENT_PLAN_2026-09-16.md)：现状审计、工作包、测试层级、性能测量与完成定义。
3. [Gen6 AI 产品访谈记录](../../../technical/GEN6_AI_PRODUCT_DISCOVERY_2026-09-16.md)：三轮用户回答、G6-R01～21 及确认/设计建议来源。

以上是需求和设计交付，不代表已有代码或真实仿真验收通过。以下为既有详细设计与证据入口：

点云感知扩展（2026-09-10，方案状态）：[点云驱动感知与 ADAS 联合分析 DDD 方案](POINT_CLOUD_PERCEPTION_DDD_PROPOSAL_2026-09-10.md)。包含当前源码事实、US-027～032、仿真/数据/源码/报告契约、实施切片及 PC-AC 验收；不代表已有点云运行验收通过。
...
最新设计复核与后续实施顺序见 [自主仿真分析 DDD 收口基线（2026-09-11）](SIMULATION_ANALYSIS_DDD_REVIEW_2026-09-11.md)：保留 Pi 唯一编排入口，按准确性收口、单案例真实闭环、联合推理、报告批次、泛化性能推进。此文明确局部测试与完整感知验收的区别。
...
| 点云输入/证据切片 | 已实现点迹输入审计、阶段 map/coverage、validated explicit lineage、run/attempt 完成帧账本、recorded↔replay 显式比较和故障隔离 batch index；HILMODEL=0 真实 runtime 仍待 |

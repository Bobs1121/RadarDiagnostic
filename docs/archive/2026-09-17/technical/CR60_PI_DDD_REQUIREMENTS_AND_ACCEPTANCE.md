...
> 2026-09-16：新增 [Gen6 产品与验收基线](../../../technical/GEN6_AI_PRODUCT_BASELINE_2026-09-16.md) 与 [开发计划](../../../technical/GEN6_AI_DEVELOPMENT_PLAN_2026-09-16.md)。本轮 G6-R/G6-AC 为三轮访谈后的补充和冲突裁决入口；原 US/FR/AC/PC-AC 保留追踪，复用证据须核对实际覆盖，不因新文档而宣布通过。
...
当前状态：`partially-verified`（runtime 已作为确定性输入进入 Pi context；AnalysisLedger 现支持
`feedback_confirmed` / `feedback_rejected` / `feedback_irrelevant` 三类用户反馈，保留 variant/source/data
binding、`created_by=user`、`evidence_layer=user_observation` 和 `runtime_eligible=false`，不会自动发布
knowledge。跨代码/需求/案例的 AI 根因解释、反馈驱动的知识发布和用户工作台闭环仍未完成）。
...
Pi dialogue/tool-end step、Analysis Trail 投影、原子写/事件审计/并发锁、真实 artifact 恢复
smoke、source-only Pi 自动创建/绑定/完成 AnalysisRun，以及 Pi ledger leaves 自动绑定当前 run/root 已完成；
Pi 可不传 run_id/ledger_root 更新当前 run，用户 decision、Workbench 和 AI 自动 hypothesis/experiment 编排仍未实现）。
...
关联：`US-014/FR-14`。  
当前状态：`partially-verified`（Hypothesis 状态/历史/支持与反证引用已有持久化能力；Pi bridge 可将
hypothesis/debug-experiment calls 自动绑定当前 AnalysisRun 并拒绝跨 run/root 覆盖；诊断 prompt 现要求最多 3 个
evidence-bound 候选并计划低成本区分实验，证据不足时允许 0 个；真实 P1 证据上的候选 rank/实验选择和根因发布门仍未验收）。
...
关联：`US-013`。  
当前状态：`partially-verified`（除 BYD_SC6H 外，配置的 gen6/reco_fw 已在独立 workspace 生成并查询
source-bound Code Context/index，capability manifest freshness=fresh；但该 variant 的 RTE 映射缺失，且没有录制输入、
Arbe preflight/runtime/replay，五类新项目验收和完整第二项目门仍未通过）。
...
当前状态：`partially-verified`（AnalysisRun 已记录首次可行动线索、full-read/replay/GDB 次数等 run metrics；
一条无 scope/tools 参数的 source-only Pi 查询还记录到索引检查 1 次、cache hit 1 次、查询成功 1 次和 5/5 未截断，
但 `time_to_first_useful_clue_sec=84.364` 只是含模型等待的单次样本，不构成性能基准。BYD_SC6H 与 gen6/reco_fw
均有隔离的 prewarm cold/hot 子路径测量。完整数据→回放→检索→LLM→报告性能、真实人工 baseline 和异构项目
端到端 percentiles 仍未验收；证据见
`docs/technical/evidence/pi_source_index_run_metrics_gen6_20260915.json`）。
...
- Given 当前 arbe/source/runtime capability 声明了 ObjectList，When 选择某个报警事件和
  ...
  定义，不在平台内复制固定结构。
- `ros-topic-inventory --inspect-message-schemas` 可选从当前 ROS workspace 的 `rosmsg show`
  生成带类型来源的有界字段目录与定义 SHA-256；字段表默认关闭完整定义输出，也不承担对象帧关联。
...
当前状态：`partially-verified`（本地 `ros-topic-inventory.v1` artifact 可经 `public-runtime-normalize`
投影为带 client observation time、sample/message hash 和动态 nested-array 字段来源的 ObjectList 行；
缺少 `message_seq` 时始终 `unbound`，跨 topic 样本不配对，topic plan role 需与当前 preflight
server/workspace identity 与同一 preflight file SHA 一致。`evidence-query --runtime-snapshot` 可按 radar 和
当前 message-definition token 有界查询 unbound 对象字段，仍显式标记没有 event/frame association。
`diagnosis-report --runtime-snapshot` 可在已有 bundle/viewer 上携带独立 snapshot rows/evidence layer，
但不把 unbound 行并入 selected event/condition trace。真实远端采样/字段目录、ego-warning-radar_info 联动、
HTML drilldown、代码条件跳转及 AnalysisRun 身份绑定
仍未现场验收。`M4-AC-002` 跟踪该本地 slice）。
...
当前状态：`partially-verified`（自然语言 source-only CLI 已自动创建并完成 AnalysisRun，绑定当前 context/index hash，
真实 Pi code-analyze 单次调用与 direct index 一致；manifest companion discovery 和 ledger recovery 已通过；
带副作用的全流程及 perception-level AI 长链仍需现场验收）。
...
| Explain/feedback | US-014 | expert panel/memory/knowledge guard + AnalysisLedger user feedback kinds | diagnosis bundle / user-observation.v1 | feedback_confirmed/rejected/irrelevant provenance-bound ledger tests; automatic knowledge publishing and Workbench feedback loop remain pending | partially-verified |
| Analysis trail | US-015 | AnalysisLedger + Pi dialogue/tool step + diagnostic-report Analysis Trail | analysis-run/step/claim | ledger recovery + source-only auto run + active-run/root bridge binding + conflict rejection + tool-end step + HTML projection | partially-verified |
...
| Root-cause loop | US-018 | hypothesis board + experiment planner | `hypothesis.v1` | hypothesis state/history + current-run bound Pi tools + cross-run conflict gate + report projection; AI candidate generation/experiment selection not verified | partial |
| Gen6 adaptation | US-019 | ProjectCapabilityManifest + plugin SPI + effective-default-variant path/domain resolver | capability manifest / variant-scoped source, DBC, source_docs and source_domains | BYD_SC6H configuration/backfill tests plus an isolated gen6/reco_fw code-context/index and fresh partial capability manifest; RTE missing, data/runtime and five-case second-project gate remain pending | partially-verified |
| Efficiency | US-020 | run metrics + incremental indexes + capability packs + prewarm cold/hot isolation | metrics/step cost + prewarm timing report | one natural source-only Pi AnalysisRun records code-index refresh/cache/query metrics and direct-index parity; isolated BYD_SC6H active RTE/COEM hash-bound prewarm plus gen6/reco_fw isolated offline-stub prewarm (1 cold + 9 warm each); full pipeline timings, real baseline and second-project data/runtime percentiles remain pending | partially-verified |
| Arbe ObjectList | US-021 | PublicRuntimeCollector + `public-runtime-normalize` + `evidence-query` + ArbeReplayAdapter | `runtime-snapshot-with-frame.v1` / collector artifact | `ros-topic-inventory.v1` is preflight-hash-bound and normalizes to client-time/sample/schema provenance; current-schema ObjectList fields can be queried by radar with an explicit unbound status; real remote sampling, ego/warning join, report drilldown, code/experiment navigation and AnalysisRun identity binding remain pending | partially-verified |
| Code context | US-022 | code-context-refresh/read + EventCodePathBuilder + capability manifest + Pi source-only auto refresh | `code-context.v1` / `code-index.v1` / `event-code-path.v1` + context/index hash binding | effective-variant source refresh/reuse, context/index root+snapshot+SHA-256 gates, child bridge injection/override rejection tests | partially-verified |
...
| Standalone Pi entry | US-024 | `PiModule` + `PiBridge` + generated registerTool + ledger + config-bound source-only context | PiRunContext / AnalysisRun / child source-index binding | real source-only CLI auto-creates a completed AnalysisRun, binds current context/index hashes, executes one inline code-analyze call, and matches direct index verification; source-only text without tool evidence stays blocked; perception-level reasoning remains partial | partially-verified |
...
### 2026-09-10 点云感知需求扩展（specified）

用户明确要求从点云重新运行前级感知，并结合源码和数据交付分析报告；保留现有 ADAS 分析。
本基线预留以下唯一编号，详细 Given/When/Then、设计和验收见
[点云感知 DDD 方案](POINT_CLOUD_PERCEPTION_DDD_PROPOSAL_2026-09-10.md)：

| 用户故事 | 范围 | 验收 | 状态 |
|---|---|---|---|
| US-027 | 审计点迹输入及可执行阶段 | PC-AC-01/02 | partially-implemented（PC-AC-01 accepted；target-only boundary 与 payload audit 显式，若有 source context 则保留静态代码候选；真实 GUI-downsampled 数据/ADAS 退化分析仍待现场验证） |
| US-028 | 编译绑定、连续点云回放、reset/预热 | PC-AC-03/04 | partially-implemented（plan/binding/run ledger 已有；新鲜 HILMODEL=0 build/replay 与 producer-owned reset/warm-up ACK 未验收） |
| US-029 | 点迹/簇/航迹生命周期和 lineage | PC-AC-05/06 | partially-implemented（canonical public lineage 与 raw/support counts 分开；相邻 callback 的同 UID recurrence 标为 derived；物理身份、ID reuse 和内部生命周期 trace 仍缺） |
| US-030 | 当前源码与实际阶段数据联合推理 | PC-AC-07/08 | partially-implemented（Pi P2 read-only 分析正确保持 `facts_only`/0 causal hypotheses；runtime 条件代入和内部阶段观察仍缺） |
| US-031 | 感知场景、阶段报告与批次索引 | PC-AC-09/10 | implemented（JSON/HTML projection + fault-isolated batch index） |
| US-032 | 双路径/跨版本比较与可反驳复验 | PC-AC-11/12 | partially-implemented（explicit comparison read model；field replay pending） |

当前 symmetry `postProcess.c` 把主要感知阶段置于 `#if 0 == HILMODEL`，已有目标注入回放和
release smoke 不能证明本扩展完成。旧 US-008 的预热 profile 实现与本扩展的真实点云验收分开记录。
...
## 10. 2026-09-09 M0–M4 本地交付实现检查点

本节记录当前工作树中已经实现并可重跑的产品化增量，不能替代真实远端和干净机器验收。
机器可读明细位于 [`release_acceptance.v1.json`](release_acceptance.v1.json)，执行入口为
`python tools/release_gate.py`。

| 阶段 | 当前实现 | 本地证据 | 现场缺口 |
|---|---|---|---|
| M0 | 唯一 DDD 基线 + release acceptance index + evidence/hash gate | `tools/release_smoke.py --phase M0`、`tests/test_release_acceptance.py`，全 release gate required `10/10` | 本次版本仍需在交付分支冻结并由 reviewer 签收 |
| M1 | variant manifest 缺失/未发布/签名失配 fail-closed；confirmed 必须经过显式 runtime/replay/user gate；缺少 Harness L2 不得通过 | `tests/test_knowledge_guard.py`、`tests/test_conclusion_publication.py`、`tests/test_harness_phase2.py` | 真实历史案例需重新生成并检查跨入口结论一致性 |
| M2 | Pi/CLI 自动生成 execution binding、唯一 attempt、远端 recorder `trap` 清理、Pi 远程执行缺 binding 时阻断 | `ai/modules/execution_binding.py`、`outputs/remote_public_execute_20260910.json`、`outputs/remote_public_interrupt_evidence_20260910.json`、`tests/test_execution_binding.py` | approved replay、SCP fetch 和 SSH 中断清理已验证；8 个 object rows 保持 `unbound`，不伪造同帧目标；中断 attempt 没有 capture JSON，因此只保留 partial bag/log |
| M3 | `tools/doctor.py` 只读检查 Python/锁定依赖/入口/64 个 capability；依赖与 CLI 可由 gate 重跑 | `requirements.lock`、`python tools/doctor.py --json` | 另一台干净机器安装和 provider/Pi 组合仍待现场验收 |
| M4 | variant/project isolation 回归纳入 release gate，能力通过现有 registry/adapter 扩展 | `tests/test_project_isolation.py`、`tests/test_project_capability.py`、gen6/reco_fw isolated context/prewarm artifacts | 第二个项目的无报警/已知报警/runtime/缺输入/版本变化及真实数据格式仍需按同一 AC 验收 |

当前 release gate 的 `partially-verified` 条目不会被计入本地首版必需门，但不能被解释为远端仿真或干净机器已经交付。

2026-09-13 复验：全量 `pytest` 为 `860 passed, 1 skipped, 2 xfailed, 10 warnings`；
`python tools/release_gate.py` accepted `10/10` required entries；`python tools/doctor.py --json`
返回 `ready`、64 capabilities、`lock_mismatches=[]`。该本地门不改变 PC-AC-03/04/05/06/07/11/12
与第二异构项目/干净机器的现场验收状态，详见点云 DDD 方案 §12.1。

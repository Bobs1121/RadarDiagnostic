...
> 2026-09-16 产品基线更新：后续开发先读 [Gen6 产品与验收基线](../../technical/GEN6_AI_PRODUCT_BASELINE_2026-09-16.md)。本文中“不自动修改工作区”等旧边界按新基线解释为禁止未授权修改；授权后的隔离修改、编译、回灌验证已纳入首期三层能力目标。范围、交互和性能冲突以新基线为准，历史证据不自动升级。

> 2026-09-10 需求增补：用户要求在 ADAS 功能分析基础上，用录制点迹重新运行前级感知，
> 将点迹处理、聚类、跟踪、目标输出与当前源码和 ADAS 结果联合解释，交付可追溯报告及批次索引。
> 范围为点迹/检测列表输入后的处理；不自动扩展为 ADC/基带算法仿真。
> 详细方案见 [点云感知 DDD 方案](technical/POINT_CLOUD_PERCEPTION_DDD_PROPOSAL_2026-09-10.md)，
> 需求编号 US-027～US-032，状态 specified；既有 SGU 回放验收不能替代点云感知验收。
